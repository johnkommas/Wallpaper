...
def randar_chart(categories, sales, path):
    ...
    for a, b, in zip(sales, categories):
        label = f"{a}\nEUR"
        # this method is called for each point
        plt.annotate(label,  # this is the text
                     (a, b),  # this is the point to label
...
def tree_map(df, path):
    ...
    try:
        ...
        labels = df.apply(
            lambda x: f'{x[0]} ({round(int(x[2]) * 100 /all["SALES"].sum())}%)\n{int(x[1])}{"T" if x[1] - int(x[1]) == 0 else "Κ"}/{int(x[2])}€',
            axis=1)
        ...
        squarify.plot(sizes=sizes, label=labels, color=colors, alpha=1, text_kwargs={'color': '#FFFFFF', 'size': 60},
                       pad=True)
        plt.axis('off')
        plt.tight_layout()
        img_file = f"{path}/tree_map.png"
...
def make_wordcloud(df, path):
    ...
    wc = WordCloud(mode="RGBA", background_color=None, font_path='Avenir Next.ttc', scale = 7, width ='1920', height='1080', max_words=2000, mask=parrot_mask, max_font_size=180, random_state=42, relative_scaling=.5)
...
def pda():
    ...
    centre_circle = plt.Circle((0, 0), .65, fc='#27282C')
    fig = plt.gcf()
